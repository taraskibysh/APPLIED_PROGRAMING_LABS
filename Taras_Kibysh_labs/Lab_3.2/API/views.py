...
class CustomerItemInsuranceView(generics.GenericAPIView, CommonDoubleMixin):
    ...
    # def get(self, request, worker_id=None, customer_id=None):
    #     if worker_id and customer_id:
    #         instance = self.repository.get_by_id(worker_id, customer_id)
    #         if instance is None:
    #             return Response(status=status.HTTP_404_NOT_FOUND)
    #
    #         serializer = self.get_serializer(instance)
    ...
    #     return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    #
    # def put(self, request, worker_id, customer_id):
    #     instance = self.repository.get_by_id(worker_id, customer_id)
    #     if instance is None:
    #         return Response(status=status.HTTP_404_NOT_FOUND)
    #
    ...
    #     return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    #
    # def delete(self, request, worker_id, customer_id):
    #     if self.repository.delete(worker_id, customer_id):
    #         return Response(status=status.HTTP_204_NO_CONTENT)
    #     return Response(status=status.HTTP_404_NOT_FOUND)
